        ...
        if (existingCompany) {
            if (existingCompany.verified === false && existingCompany.otp) {
                return res.status(400).json({
                    error: "Multiple registration attempts detected. Please wait 10 minutes before trying again. Still facing the same problem? Contact us.",
                });
            }
...
router.post("/company-login", async(req, res) => {
    try {
        ...
        const token = jwt.sign({ companyId: company._id, email: company.officialEmail },
...
router.get("/student/:studentID", verifyEmployerToken, async(req, res) => {
    try {
        ...
        const student = await Student.findOne({ studentID: studentID }).select("-password -__v");
